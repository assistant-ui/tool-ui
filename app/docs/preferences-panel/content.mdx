...
Use `PreferencesPanelReceipt` to render a confirmed, read-only summary once the user has saved.
...
import {
  ...
  PreferencesPanelErrorBoundary,
  parseSerializablePreferencesReceipt,
  SerializablePreferencesReceiptSchema,
  type SerializablePreferencesReceipt,
} from "@/components/tool-ui/preferences-panel";

export const PreferencesPanelReceiptTool =
  makeAssistantTool<SerializablePreferencesReceipt>({
    toolName: "show_preferences_receipt",
    description: "Show confirmed preference selections",
    parameters: SerializablePreferencesReceiptSchema,
    render: ({ args, toolCallId }) => {
      const data = parseSerializablePreferencesReceipt({
        ...args,
        id: (args as any)?.id ?? `preferences-panel-${toolCallId}`,
      });
