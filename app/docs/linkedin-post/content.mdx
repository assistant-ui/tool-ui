import { DocsHeader } from "../_components/docs-header";
import { LinkedInPost } from "@/components/tool-ui/linkedin-post";

<DocsHeader
  title="LinkedIn Post"
  description="Platform-native post card for LinkedIn."
  mdxPath="app/docs/linkedin-post/content.mdx"
/>

<Tabs items={["Preview", "Code"]}>
  <Tab>
    <div className="not-prose w-full max-w-[540px]">
      <LinkedInPost
        post={{
          id: "linkedin-post-1",
          author: {
            name: "Athia Zohra",
            avatarUrl:
              "https://images.unsplash.com/photo-1740510294141-33ebf26e0809?q=80&w=987&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
            headline: "Engineering Lead | Building AI products",
          },
          text: "Excited to share that our team shipped a major update today. Six months of hard work paid off!",
          stats: { likes: 234 },
          createdAt: "2025-01-05T09:00:00.000Z",
        }}
      />
    </div>
  </Tab>
  <Tab>
    ```tsx
    import { LinkedInPost } from "@/components/tool-ui/linkedin-post";

    export function Example() {
      return (
        <LinkedInPost
          post={{
            id: "linkedin-post-1",
            author: {
              name: "Athia Zohra",
              avatarUrl: "https://images.unsplash.com/photo-1740510294141-33ebf26e0809?q=80&w=987&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
              headline: "Engineering Lead | Building AI products",
            },
            text: "Excited to share that our team shipped a major update today. Six months of hard work paid off!",
            stats: { likes: 234 },
            createdAt: "2025-01-05T09:00:00.000Z",
          }}
        />
      );
    }
    ```
  </Tab>
</Tabs>

## Key Features

<FeatureGrid>
  <Feature icon="Paintbrush" title="Platform-native styling">
    Matches LinkedIn visual language and interaction patterns
  </Feature>
  <Feature icon="FileText" title="Long-form post support">
    Optimized layout for professional and text-heavy content
  </Feature>
  <Feature icon="MousePointerClick" title="Response actions">
    Add custom actions below each post
  </Feature>
  <Feature icon="Link" title="Link previews">
    Supports rich previews with title, image, and domain metadata
  </Feature>
</FeatureGrid>

## Source and Install

Install this component directly from the Tool UI shadcn registry:

```bash
npx shadcn@latest add https://tool-ui.com/r/linkedin-post.json
```
If you're in a pnpm workspace, run the command in the package directory that contains `components.json`:

```bash
npx shadcn@latest add --cwd <path-to-package-with-components-json> https://tool-ui.com/r/linkedin-post.json
```

This command installs `components/tool-ui/linkedin-post`, `components/tool-ui/shared`, and `lib/ui/cn.ts` into your project.

## Usage

```tsx
import { type Toolkit } from "@assistant-ui/react";
import {
  LinkedInPost,
  safeParseSerializableLinkedInPost,
} from "@/components/tool-ui/linkedin-post";
import { createResultToolRenderer } from "@/components/tool-ui/shared";

export const toolkit: Toolkit = {
  showLinkedInPost: {
    type: "backend",
    render: createResultToolRenderer({
      safeParse: safeParseSerializableLinkedInPost,
      render: (parsedResult) => <LinkedInPost post={parsedResult} />,
    }),
  },
};
```

## Props

<TypeTable
  type={{
    post: {
      description: "Post payload validated by `SerializableLinkedInPostSchema`",
      type: "LinkedInPostData",
      required: true,
    },
    className: {
      description: "Optional className for the outer card",
      type: "string",
    },
    onAction: {
      description: "Fires when the user triggers a built-in post action",
      type: "(action: string, post: LinkedInPostData) => void",
    },
    responseActions: {
      description: "Optional custom response actions",
      type: "Action[]",
    },
    onResponseAction: {
      description: "Fires when a response action is selected",
      type: "(actionId: string) => void | Promise<void>",
    },
    onBeforeResponseAction: {
      description: "Optional preflight guard before a response action runs",
      type: "(actionId: string) => boolean | Promise<boolean>",
    },
  }}
/>
