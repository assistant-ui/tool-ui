...
Long-running operations need progress visibility. ProgressTracker shows each step of a multi-step workflow (build, test, deploy) with its current status: pending, in-progress, completed, or failed. The user sees exactly where the operation is and what went wrong if it breaks. Once it finishes, the tracker collapses into a receipt with the final outcome.
...
export const toolkit: Toolkit = {
  trackProgress: {
    ...
    render: createResultToolRenderer({
      ...
      render: (parsedResult) => <ProgressTracker {...parsedResult} />,
    }),
  },
};
```
...
Pass `choice` to render the final state after an operation completes, fails, or is cancelled. The receipt shows all steps with their final statuses, an elapsed time badge, and an outcome indicator in the top-right corner. Supports `success`, `partial`, `failed`, and `cancelled` outcomes. Read-only with no action buttons.
...
<TypeTable
  type={{
    id: {
      description: "Unique identifier for the step (must be unique within the steps array)",
      type: "string",
      required: true,
    },
    label: {
