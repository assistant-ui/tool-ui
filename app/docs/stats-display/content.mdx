...
```bash
npx shadcn@latest add --cwd <path-to-package-with-components-json> https://tool-ui.com/r/stats-display.json
```

This command installs `components/tool-ui/stats-display`, `components/tool-ui/shared`, and `lib/ui/cn.ts` into your project.
...
export const toolkit: Toolkit = {
  showStats: {
    ...
    render: createArgsToolRenderer({
      safeParse: safeParseSerializableStatsDisplay,
      idPrefix: "stats",
      render: (parsedArgs) => (
          <StatsDisplay {...parsedArgs} />
      ),
    }),
  },
};
