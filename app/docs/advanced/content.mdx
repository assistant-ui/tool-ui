...
The [Quick Start](/docs/quick-start) covers the basics: installing a component, registering it, and seeing it render. This page goes further: stronger typing with `InferUITools`, frontend tools that run entirely in the browser, and patterns for larger integrations like auto-continue and edge runtime forwarding.
...
Decision surfaces like OptionList are best modeled as **frontend tools**: the model calls a tool whose arguments are the component props, and your UI calls `addResult(...)` after the user confirms. The result feeds back into the conversation as a tool result.
...
When a tool call produces results that need further processing (say, a search that should be followed by a summary) auto-continue tells the assistant to keep going instead of stopping and waiting for user input. Enable it with `sendAutomaticallyWhen`:
