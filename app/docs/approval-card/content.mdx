...
When `decision` is set, the component renders in a compact receipt state showing the approval decision. This is useful for displaying the outcome in conversation history.
...
>({
  ...
  render: ({ args, result, addResult, toolCallId }) => {
    ...
    return (
      <ApprovalCardErrorBoundary>
        {result !== undefined ? (
          <ApprovalCard {...approvalCard} decision={result} />
        ) : (
          <ApprovalCard
            {...approvalCard}
