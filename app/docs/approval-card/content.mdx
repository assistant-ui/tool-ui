...
Deploying to production, deleting a project, sending an email. Consequential actions deserve a gate. ApprovalCard renders a confirmation prompt with context about the action, and records the decision as a receipt once the user responds.
...
Pass `decision` to render the component as a compact receipt. The receipt shows the outcome (approved or denied) with the custom button label you specified, fades in with a subtle animation, and is read-only.
