...
```
If you're in a pnpm workspace, run the command in the package directory that contains `components.json`:

```bash
npx shadcn@latest add --cwd <path-to-package-with-components-json> https://tool-ui.com/r/plan.json
```

This command installs `components/tool-ui/plan`, `components/tool-ui/shared`, and `lib/ui/cn.ts` into your project.
