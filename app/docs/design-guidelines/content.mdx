...
- **`surfaceId`**: Identifies this specific rendering in the conversation ("the preview above", "that table").
...
- **Redundant narration:** The assistant shouldn't describe what the surface already shows. Conversely, the surface shouldn't echo what the assistant just said. If the assistant asks "Where to?", the surface doesn't need a header saying "Where to?" — just show the options. Divide the work: the assistant provides context, the surface provides structure.
