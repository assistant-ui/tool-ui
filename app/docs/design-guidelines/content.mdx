...
Tool UI surfaces **must** be composed exclusively from the shadcn/ui wrappers defined in `components/ui/*`.  
This keeps styling and accessibility consistent across instances and lets us theme everything centrally.
...
- `Tooltip`, `Badge`, `Avatar`, `Accordion`, `Item`

> Need a new primitive? Add the shadcn wrapper under `components/ui/` and document it before using it in Tool UIs.

### Fork, edit, promote

- Use `pnpm fork:instance <source> <target>` to clone a manifest for a new experiment.
- Draft schema or UI changes locally via the JSON editor in the instance view (stored in localStorage).
- Run `pnpm cloud:deploy <slug> <deploymentId>` (or `cloud:promote`) to track Cloud deployments in the manifest once stable.
...
Use [Contextual Actions](/docs/contextual-actions) as the default CTA strip under Tool UIs.

- Keep 2–3 actions tied to the content; prefer `layout="inline"` and right alignment unless mobile demands stacking.
- Use `confirmLabel`/`onBeforeAction` for destructive or high-impact steps.
- If users must pick options first, pair the CTA row with controls (filters, `OptionList`) instead of expanding the CTA bar.
