...
In AI chat, users interact with both the assistant and any tool UIs it renders. Most tool UIs treat this relationship as incidental: a tool UI appears, the user clicks something, done. We think it matters more than that. The assistant, the tool UI, and the user form a collaborative triad.

<CollaborationDiagram />

The assistant contextualizes, interprets, and narrates. The tool UI provides structure that prose cannot: sortable tables, precise controls, rich media. They work together.
...
- **Decision:** Capture choices that matter (approve/reject, send/cancel). These need clear options and receipts that prove what happened.
...
Use stable IDs from your backend (database IDs, canonical URLs) rather than array indexes or UUIDs generated at render time. Anything the user can act on should have an ID the assistant can cite.
...
See the [Receipts](/docs/receipts) page for detailed guidance on receipt states and how to write receipt copy.
...
- **Redundant narration:** The assistant shouldn't describe what the tool UI already shows. The tool UI shouldn't echo what the assistant just said. If the assistant asks "Where to?", the tool UI doesn't need a header saying "Where to?"; just show the options. Divide the work: the assistant provides context, the tool UI provides structure.
