...
🔴 **Required** — Hard invariant; must pass to be a Tool UI  
🟡 **Default Budget** — Recommended constraint; bend with clear reason

- **One intent per surface** 🔴
- **Inline, not destination** 🔴
- **Schema-first & addressable** 🔴
- **Assistant-anchored & conversation-ready** 🔴
- **Lifecycle & time are explicit** 🔴
- **Effects require receipts** 🔴
- **Respect budgets** (3s / 3 choices / 2 levels) 🟡
- **Accessibility floor** 🔴
...
- **Role:** information  
- **Lifecycle:** invocation → output-pending → interactive
...
- **During:** Explain transitions → *"Filtered to high-priority items"*
- **After:** Summarize outcomes → *"Approved and merged to main branch"*

### 5. Temporal & Lifecycle Awareness

Tool UIs move through explicit phases: invocation → loading → interactive → commit → receipt.
...
1. **Invocation** → Shell appears, intent clear
2. **Output-pending** → Skeleton matches eventual layout
3. **Interactive** → User can act
4. **Committing** → Progress for side effects
5. **Receipt** → Durable summary
6. **Errored** → Actionable error surface
...
| **Stale** | ⏰ "As of 2:41 PM" | "This is 20 min old—refresh?" |
| **Historical** | Collapsed | "As we discussed earlier..." |
| **Superseded** | ↓ "Updated below" | "I've updated this with..." |

**Update Strategy:**
- Minor recent changes → update in place
- Major changes or time gap → new surface, mark old superseded
...
Full specification: [UI Specification → Base Schema](./ui-specification)
...
<div className="not-prose">
  <div className="flex flex-col items-center space-y-4 p-6">
    ...
    <div className="flex items-center space-x-8">
      ...
      <div className="text-sm">Direct Click ↘</div>
    </div>
...
Every important UI action ↔ one clear canonical sentence:
...
**Update strategies:**
- Minor edits → update in place, narrate diff
- Major changes → new surface, mark previous historical

## Budgets & Quality

### Cognitive Load 🟡

- **3-second glance** — Role & primary action clear
- **≤3 primary choices** — Visible without scrolling
- **≤2 disclosure levels** — Surface → details (no deeper)

### Performance 🟡

- **First content ≤300ms** — After stream start
- **No container resize** — During streaming
- **Transitions ≤200ms** — Respect reduced motion

### Density 🟡

- **No inner scrollbars** — Let chat handle scrolling
- **"View more" pattern** — For large content

## Accessibility & Responsiveness

### Required Floor 🔴

- **Keyboard** — Everything reachable, logical tab order
- **Screen readers** — Roles, labels, state changes announced
- **Visual** — AA contrast, 44px minimum touch targets
- **Mobile-first** — Works at 320px, scales up gracefully

## Anti-Patterns

### Ban List

- **Form-in-chat** — Multi-field forms pretending to be Tool UIs
- **Hidden side effects** — Mutations without receipts
- **Kitchen-sink surfaces** — Dashboards in messages
- **Layout jump** — Container bounds changing mid-stream
- **Tool UI as destination** — Navigation flows inside messages
...
You still use Radix/shadcn under the hood—Tool UI makes them conversation-native.
...
### Quality 🟡
- [ ] **Cognitive budget** — 3s glance, ≤3 choices, ≤2 levels?
