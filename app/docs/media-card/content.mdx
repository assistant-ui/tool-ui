...
- **Not for:** Multiple items at once (use Item Carousel), large galleries, long-form content editing

## Source and Install

Copy `components/tool-ui/media-card` and the `shared` directory into your project. The `shared` folder contains utilities used by all Tool UI components. The `tool-ui` directory should sit alongside your shadcn `ui` directory.
