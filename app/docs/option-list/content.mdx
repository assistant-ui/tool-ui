...
Sometimes the agent needs the user to pick: output format, plan tier, movie genre. OptionList renders selectable options with single or multi-select behavior. The selection feeds back to the assistant as a tool result, and the chosen option stays visible as a receipt. Without it, the agent would have to ask for a text reply and parse the answer.
...
Pass `choice` to render a read-only receipt showing only the selected option(s) with a checkmark. Unselected options and action buttons are hidden.
