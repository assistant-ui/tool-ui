...
export function Example({ payload }: { payload: React.ComponentProps<typeof OptionList> }) {
...
These snippets use [assistant-ui](https://assistant-ui.com) for end-to-end wiring, but the OptionList component itself is framework-agnostic at the UI layer: you can use it in any React codebase with any LLM SDK or tool-calling interface that can provide compatible props.
