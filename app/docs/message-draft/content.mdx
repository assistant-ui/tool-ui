...
Sending a message on someone's behalf is a high-stakes action. MessageDraft shows the full email or Slack message for review, gives the user a Send button with an undo grace period, and collapses into a compact receipt once sent or cancelled. Without it, the agent would send silently or dump the draft as plain text.
