...
<DocsHeader title="Contextual Actions" role="Controls" />
...
// Config form
{
  ...
  layout: "inline",    // default "inline" (use "stack" to force vertical)
...
<DataTable
  ...
  onTableAction={(id) => console.log(id)}
/>
```
...
- `layout="stack"` for tight mobile contexts or when labels wrap; `inline` + `align="right"` for standard CTA rows.
