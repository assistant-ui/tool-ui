...
Agents regularly surface tabular data: search results, issue lists, transaction logs. DataTable renders those rows as a sortable table on desktop and stacked cards on mobile. You define columns with declarative formatting configs: currency, dates, status badges, links. Without it, you would dump JSON or build a custom table from scratch.
