...
Tool UI publishes every component as a shadcn registry entry under `https://tool-ui.com/r/<component>.json`. That JSON snippet bundles the component code, the shared helpers from `components/tool-ui/shared`, and the common `lib/ui/cn.ts` utility.
...
The CLI will copy `components/tool-ui/plan`, `components/tool-ui/shared`, and `lib/ui/cn.ts` into your project and wire up the right Tailwind styles, just like any other shadcn block.
...
Tool UI components are installed from registry entries, and each entry includes `components/tool-ui/shared` and `lib/ui/cn.ts` automatically.
