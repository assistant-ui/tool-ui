...
Render interactive, accessible UI in chat from JSON tool outputs.

Why teams reach for Tool UI:
...
Try it: [Gallery presets](/docs/gallery).
...
## Choose the right Tool UI for the job

Need help deciding between Data Table, Media Card, Decision Prompt, Social Post, or the Markdown table pattern? Jump to the [Overview → Choose the right Tool UI for the job](/docs/overview#choose-the-right-renderer) section for chat-specific guidance.
