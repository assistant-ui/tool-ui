...
Some tasks require multiple decisions in sequence: project setup, onboarding, triage. QuestionFlow walks the user through single- or multi-select steps with back/next navigation. You can define all steps upfront or let the AI generate each step based on previous answers.
...
Pass `choice` with a title and summary to display the completed configuration. The receipt is read-only and shows a "Complete" badge with all choices listed.
