...
```bash
npx shadcn@latest add --cwd <path-to-package-with-components-json> https://tool-ui.com/r/question-flow.json
```

This command installs `components/tool-ui/question-flow`, `components/tool-ui/shared`, and `lib/ui/cn.ts` into your project.
...
Register this toolkit with `Tools({ toolkit })` inside your runtime provider.
