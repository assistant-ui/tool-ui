...
import { memo, useId, useState, useEffect, useRef, type FC } from "react";
...
const sdkGuideComponents = memoizeMarkdownComponents({
  h1: ({ className, ...props }) => (
    <h1
      className={cn(
        "aui-md-h1 mb-4 scroll-m-20 text-xl font-bold tracking-tight last:mb-0",
        className
      )}
      {...props}
    />
  ...
  h3: ({ className, ...props }) => (
    <h3
      className={cn(
        "aui-md-h3 mt-4 mb-2 scroll-m-20 text-base font-semibold tracking-tight first:mt-0 last:mb-0",
        className
      )}
      {...props}
    />
  ...
  h4: ({ className, ...props }) => (
    <h4
      className={cn(
        "aui-md-h4 mt-4 mb-2 scroll-m-20 text-sm font-semibold tracking-tight first:mt-0 last:mb-0",
        className
      )}
      {...props}
    />
  ...
  blockquote: ({ className, ...props }) => (
    <blockquote
      className={cn(
        "aui-md-blockquote border-l-2 pl-4 italic text-muted-foreground",
        className
      )}
      {...props}
    />
  ...
  th: ({ className, ...props }) => (
    <th
      className={cn(
        "aui-md-th bg-muted px-3 py-1.5 text-left text-xs font-semibold first:rounded-tl-md last:rounded-tr-md",
        className
      )}
      {...props}
    />
