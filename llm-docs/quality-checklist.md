...
Adjust paths to your project layout. Prefer adding these under **tests**/.

6.1 sortData edge cases (Jest/TS)

// **tests**/sortData.spec.ts
...
// **tests**/DataTableHeader.a11y.spec.tsx
...
with: { node-version: 20, cache: 'npm' } - run: npm ci - run: npm run typecheck - run: npm run lint - run: npm run test -- --ci - run: npm run test:e2e -- --reporter=line || true # allow flaky env, report anyway
