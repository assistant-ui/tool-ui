...
import { resolveWeatherEffectsCanvasProps } from "../weather-authoring/weather-widget/effects/canvas-resolver";
import { createStudioTimestamp } from "../weather-authoring/weather-widget/effects/canvas-resolver-base";
import {
  TIME_CHECKPOINT_ORDER,
  TIME_CHECKPOINTS,
  type TimeCheckpoint,
  type WeatherEffectsTunedPresets,
} from "../weather-authoring/weather-widget/effects/tuning";
import type { WeatherConditionCode } from "../weather-authoring/weather-widget/schema";
...
const DISCRETE_PRESETS_OUTPUT_RELATIVE =
...
function buildDiscreteWeatherCanvasPresets(
  authoringPreset: unknown,
): Record<WeatherConditionCode, Record<TimeCheckpoint, unknown>> {
  const tunedPresets = authoringPreset as WeatherEffectsTunedPresets;
  const byCondition = {} as Record<
    WeatherConditionCode,
    Record<TimeCheckpoint, unknown>
  >;

  for (const conditionCode of WEATHER_CONDITION_CODES) {
    const byCheckpoint = {} as Record<TimeCheckpoint, unknown>;

    for (const checkpoint of TIME_CHECKPOINT_ORDER) {
      const timeOfDay = TIME_CHECKPOINTS[checkpoint];
      byCheckpoint[checkpoint] = resolveWeatherEffectsCanvasProps({
        conditionCode,
        timeOfDay,
        timestamp: createStudioTimestamp(timeOfDay, "2026-01-01T00:00:00Z"),
        tunedPresets,
        checkpointMode: "nearest",
      });
    }

    byCondition[conditionCode] = byCheckpoint;
  }

  return byCondition;
}

export function compileDiscreteWeatherCanvasPresetModule(
  authoringPreset: unknown,
): string {
  const discretePresets = buildDiscreteWeatherCanvasPresets(authoringPreset);
  const canonical = canonicalizeWeatherPresetData(
    stripUndefinedDeep(discretePresets),
  );

  return [
    "// AUTO-GENERATED by `pnpm weather:compile`.",
    "// Source: lib/weather-authoring/presets/tuned-presets.json + effects/canvas-resolver.ts",
    "// DO NOT EDIT MANUALLY.",
    "",
    'import type { WeatherConditionCode } from "../../schema-runtime";',
    'import type { TimeCheckpoint } from "../tuning";',
    'import type { WeatherEffectsCanvasProps } from "../weather-effects-types";',
    "",
    "export const DISCRETE_WEATHER_EFFECTS_CANVAS_PRESETS: Record<",
    "  WeatherConditionCode,",
    "  Record<TimeCheckpoint, WeatherEffectsCanvasProps>",
    `> = ${serializeJsLiteral(canonical)};`,
    "",
  ].join("\n");
}
...
export async function buildWeatherRuntimeArtifacts(
  ...
  const nonBundledArtifacts: WeatherRuntimeArtifact[] = [
    ...
    {
      relativePath: DISCRETE_PRESETS_OUTPUT_RELATIVE,
      contents: compileDiscreteWeatherCanvasPresetModule(authoringPreset),
    },
    {
