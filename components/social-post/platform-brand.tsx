...
const InstagramLogo: React.FC<LogoProps> = ({ className }) => {
  const rawId = React.useId();
  const base = React.useMemo(() => sanitizeId(`ig-${rawId}`), [rawId]);
  const gradC = `${base}-c`;
  const gradD = `${base}-d`;
...
const TikTokLogo: React.FC<LogoProps> = ({ className }) => (
  <svg
...
const LinkedinLogo: React.FC<LogoProps> = ({ className }) => (
  <svg
    viewBox="0 0 72 72"
    width="100%"
    height="100%"
...
const LOGO_MAP: Record<Platform, React.FC<LogoProps>> = {
  x: XLogo,
