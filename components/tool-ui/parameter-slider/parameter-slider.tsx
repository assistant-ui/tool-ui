...
function SliderRow({ config, value, onChange, disabled }: SliderRowProps) {
  ...
  useLayoutEffect(() => {
    ...
    setGap(Math.max(labelGap, valueGap));
  }, [value, min, max]);
  ...
  const ticks = useMemo(() => {
    ...
    return Array.from({ length: TICK_COUNT }, (_, i) => {
      ...
      const value = min + range * (i / (TICK_COUNT - 1));
      const isZero = crossesZero && Math.abs(value) < range * 0.01;
          ...
          {ticks.map((tick, i) => {
            ...
            return (
              <span
                key={i}
                className={cn(
                  "pointer-events-none absolute bottom-0 h-2 w-px",
                  isFirst || isCenter
                    ? "bg-transparent"
                    : tick.isZero
