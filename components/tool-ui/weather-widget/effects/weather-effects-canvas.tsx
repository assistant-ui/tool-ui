...
// Pass 4: Lightning (illuminates scene)
...
uniform bool u_autoMode;
uniform float u_autoInterval;
...
void main() {
  ...
  // Calculate flash intensity based on time since last flash
  float timeSinceFlash = u_time - u_lastFlashTime;
  float flash = 0.0;

  if (timeSinceFlash < 0.1) {
    // Main flash
    flash = (1.0 - timeSinceFlash / 0.1) * u_flashIntensity;
  } else if (timeSinceFlash < 0.3) {
    // Afterglow
    flash = (1.0 - (timeSinceFlash - 0.1) / 0.2) * u_flashIntensity * 0.3;
  }

  // Illuminate the scene
  vec3 color = scene.rgb;
  color += vec3(0.8, 0.85, 1.0) * flash * u_sceneIllumination;
...
export function WeatherEffectsCanvas({
  ...
  const render = useCallback(() => {
    ...
    if (props.layers.lightning && programs.lightning) {
      ...
      gl.uniform1i(gl.getUniformLocation(programs.lightning, "u_autoMode"), p.autoMode ? 1 : 0);
      gl.uniform1f(gl.getUniformLocation(programs.lightning, "u_autoInterval"), p.autoInterval);
